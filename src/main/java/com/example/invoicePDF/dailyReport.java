...
public class dailyReport {
	public static void main(String[] args) throws IOException, ClassNotFoundException, SQLException {
        ...
        int lastMonth = MON;
        int lastYear  = YY;
        String fileName = "Daily-"+String.valueOf(lastDay)+String.valueOf(lastMonth)+String.valueOf(lastYear)+".pdf";
		...
		ArrayList<BilledProduct> bp = items;
		...
		int sn=1;
		int y=670;
//		BilledProduct bproduct = new BilledProduct();
		ResultSet rs = bpdao.count();
		while(rs.next()) {
			...
			stream.showText(rs.getString(1));
			stream.endText();
			
			stream.beginText();
			stream.setFont(PDType1Font.TIMES_ROMAN, 10);
			stream.newLineAtOffset(140+180, y);
			stream.showText(String.valueOf(pdao.getprice(rs.getString(1))));
			stream.endText();
			
			stream.beginText();
			stream.setFont(PDType1Font.TIMES_ROMAN, 10);
			stream.newLineAtOffset(220+180, y);
			stream.showText(String.valueOf(rs.getInt(2)));
			stream.endText();
			
			stream.beginText();
			stream.setFont(PDType1Font.TIMES_ROMAN, 10);
			stream.newLineAtOffset(500, y);
			stream.showText(String.valueOf(pdao.getprice(rs.getString(1))*rs.getInt(2)));
			total +=pdao.getprice(rs.getString(1))*rs.getInt(2);
			stream.endText();	
			y-=20;
			sn++;
		}
		...
		if(lastMonth == MON) {
			...
			fileName  = String.valueOf(DD)+":"+String.valueOf(MON)+":"+String.valueOf(YY)+".pdf";
			document.save(fileName);
			lastMonth = MON;
		}
	}
